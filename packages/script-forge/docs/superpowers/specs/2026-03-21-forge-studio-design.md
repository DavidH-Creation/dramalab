...
└── plugins/
    └── script-forge/   # Adapter to existing script-forge Python package
```
...
### 3.2 Script-Forge Plugin

The v1 plugin wraps the existing `script-forge` Python package.

#### 3.2.1 Required Changes to `script-forge` Core

The plugin adapter requires these targeted changes to the existing `script-forge` package:
...
- **`initialize()`** — Full workspace setup sequence:
  1. Create workspace directory + subdirs (`input/`, `sequences/`, `derived/`, `experiments/`, `exports/`, `.script-forge/`)
...
User edits criteria + sets config, clicks START
  → POST /api/plugins/script-forge/init {input_text, criteria_text, config}
  → Returns {session_id, sequences: [...]}
  → POST /api/plugins/script-forge/{session_id}/run
  → Frontend connects to GET /api/plugins/script-forge/{session_id}/stream
...
User clicks STOP
  → POST /api/plugins/script-forge/{session_id}/stop
  → SSE event: complete

User clicks Export
  → GET /api/plugins/script-forge/{session_id}/export → downloads .docx
```
...
dependencies = [
    ...
    "script-forge>=0.1.0",       # local dependency
]
...
### v1 In Scope
- Script-Forge plugin only (other tabs visible but disabled)
...
- Cloud deployment
- Database (state lives in script-forge workspace on disk)
