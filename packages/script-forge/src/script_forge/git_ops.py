...
def git_init(workspace: Path) -> None:
    ...
    subprocess.run(
        ["git", "config", "user.email", "script-forge@local"],
        cwd=workspace, capture_output=True, check=True,
    )
    subprocess.run(
        ["git", "config", "user.name", "Script-Forge"],
        cwd=workspace, capture_output=True, check=True,
    )
    subprocess.run(["git", "add", "."], cwd=workspace, capture_output=True, check=True)
    subprocess.run(
        ["git", "commit", "-m", "init: script-forge workspace"],
        cwd=workspace, capture_output=True, check=True,
    )
...
def _mark_last_experiment_crashed(workspace: Path) -> None:
    ...
    history_path = workspace / ".script-forge" / "history.jsonl"
