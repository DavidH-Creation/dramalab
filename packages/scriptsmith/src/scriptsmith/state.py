...
def save_state(workspace: Path, state: ProjectState) -> None:
    """Atomically save state to .scriptsmith/state.json."""
    content = json.dumps(state.to_dict(), ensure_ascii=False, indent=2)
    atomic_write(workspace / ".scriptsmith" / "state.json", content)


def load_state(workspace: Path) -> ProjectState | None:
    """Load state from .scriptsmith/state.json. Returns None if missing."""
    path = workspace / ".scriptsmith" / "state.json"
    if not path.exists():
        return None
...
def acquire_lock(workspace: Path) -> None:
    ...
    lock_path = workspace / ".scriptsmith" / ".lock"
    ...
    if lock_path.exists():
        ...
        try:
            ...
            if _is_process_alive(pid):
                raise RuntimeError(
                    f"Another scriptsmith process is already running (PID {pid})"
                )
        except ValueError:
            pass
