...
export function RoundTimeline({ rounds }: RoundTimelineProps) {
  ...
  return (
    <div className="space-y-2">
      <div className="text-[11px] text-[hsl(230,20%,30%)] uppercase tracking-wider mb-2">Round log</div>
      {reversed.map((round) => (
...
function RoundCard({ round }: { round: RoundResult }) {
  ...
  const borderColor =
    round.status === 'keep'
      ...
      : round.status === 'discard'
        ? 'border-l-red-500'
        : 'border-l-yellow-400';

  const statusBg =
    round.status === 'keep'
      ? 'bg-[hsl(160,30%,15%)] text-[hsl(var(--primary))]'
      : round.status === 'discard'
        ? 'bg-[hsl(350,30%,15%)] text-red-400'
      ...
      {expanded && (
        <div className="px-3 pb-3 border-t border-[hsl(230,20%,15%)] mt-0 pt-2.5 text-xs text-[hsl(var(--muted-foreground))] leading-relaxed">
          ...
          <div className="flex flex-wrap gap-1.5 mt-2">
            {Object.entries(round.scores_after).map(([dimension, score]) => {
              const before = round.scores_before[dimension];
              const changed = score !== before;
              return (
                <span key={dimension} className="bg-[hsl(230,40%,6%)] border border-[hsl(230,20%,15%)] rounded px-2 py-0.5 text-[10px]">
                  {dimension}{' '}
                  {changed ? (
                    <span className="text-[hsl(var(--primary))]">{before} to {score}</span>
                  ) : (
                    <span>{score}</span>
                  )}
                </span>
